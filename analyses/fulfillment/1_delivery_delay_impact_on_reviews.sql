...
-- Business question:
-- Does customer satisfaction deteriorate as delivery delays increase?
...
SELECT
    ...
    CASE
        WHEN customer_delivery_date.date <= estimated_delivery_date.date
            THEN 'On Time'

        WHEN customer_delivery_date.date <= estimated_delivery_date.date + INTERVAL '3 DAYS'
            THEN '1 to 3 days late'

        WHEN customer_delivery_date.date <= estimated_delivery_date.date + INTERVAL '7 DAYS'
            THEN '4 to 7 days late'

        WHEN customer_delivery_date.date <= estimated_delivery_date.date + INTERVAL '14 DAYS'
            THEN '8 to 14 days late'
...
WHERE
    f.fulfillment_sequence_anomaly_flag = 0
    AND customer_delivery_date.date IS NOT NULL
    AND estimated_delivery_date.date IS NOT NULL

GROUP BY 1

ORDER BY
    CASE delivery_segment
        WHEN 'On Time' THEN 1
        WHEN '1 to 3 days late' THEN 2
        WHEN '4 to 7 days late' THEN 3
        WHEN '8 to 14 days late' THEN 4
